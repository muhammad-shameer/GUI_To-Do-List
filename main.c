...
// Update progress bar with a count e.g 1/2 3/5 etc
void update_progress() {
    ...
    char buf[64];
    snprintf(buf, sizeof(buf), "%d / %d done", done_count, total);
...
void add_task(GtkWidget *widget, gpointer treeview) {
    ...
    time_t due = time(NULL) + 86400 * 7; // 7 days is a default due
...
int main(int argc, char *argv[]) {
    ...
    col = gtk_tree_view_column_new_with_attributes("Task", renderer, "text", 0, NULL);
    ...
    load_tasks();
    update_progress(); // makes sure the bar shows correct info on start
